...
RDFJSObject = Class.extend({
	...
	return $(this.get_all_complete(prop)).map(function(i, el) {return el.processed;});
    },
	...
	return this.get_one(prop, function(el) {return el.value;});
    },

    get_uri: function(prop) {
	return this.get_one(prop, function(el) {return el.resource.uri;});
    },

    get_resource_object: function(prop) {
	return this.get_one(prop, function(el) {return el;});
    },

    get_raw: function(prop) {
	return this.get_one_complete(prop, function(el) {return el.raw;});
    },
...
RDFJSObject.getObject = (function() {
	...
	if (!return_object) {
		...
		if (!OBJECT_TYPES[tripleObject.uri]) {
		    alert('problem with ' + tripleObject.uri + " and subject is " + subject + "," + tripleObject);
		}
...
TextAnswerJSObject = AnswerJSObject.extend({
	...
	// maybe a datatype
	this.datatype = this.get_uri(RDF.NS.survey('datatype'));
    },
...
/* Validation function for text that returns no error message or an error message
 * if the format is incorrect.
 */
...
function textOptionalValidation(input) {
    return null;
}
...
SurveyJSObject.NS = new Namespace("http://indivo.org/survey/vocab#");
...
// fixme: this should only be the URI
VALIDATION['xsd:integer'] = validate_integer;
VALIDATION['http://www.w3.org/2001/XMLSchema#integer'] = validate_integer;

VALIDATION['xsd:text'] = textValidation;
