...
//
// Some IE specific hacks by Ben Adida (ben@adida.net)
//

// get an attribute node with the given namespace URI and suffix.
function XML_getAttributeNodeNS(frame, node, prefix, suffix) {
  // built-in?
  if (node.getAttributeNodeNS) {
    return node.getAttributeNodeNS(prefix, suffix);
  }

  var prefixed_attr_value= frame.prefixes[prefix] + ':' + suffix;

  var return_val = node.getAttributeNode(prefixed_attr_value);
  //alert('returning ' + return_val);
  return return_val;
}

// copy an object, shallow (Ben)
function OBJECT_copy(obj) {
  var return_obj = {};
  for (i in obj) {return_obj[i] = obj[i];}
  return return_obj;
}

// function to update a frame's prefixes (Ben)
function frame_build_ns(parent, frame, element) {
  // not needed, speed things up
  ...
  // copy the objects
  frame.namespaces = OBJECT_copy(parent.namespaces);
  frame.prefixes = OBJECT_copy(parent.prefixes);

  // go through each one, looking for xmlns declarations
  for (var i=0; i < attrs.length; i++) {
    if (attrs[i].name.substring(0,5) == "xmlns") {
      var prefix;
      ...
      frame.namespaces[prefix] = uri;
      frame.prefixes[uri] = prefix;
    }
  }
}
...
function RDFParser(store) {
	...
	var elementURI = function (el) {
	    // localName doesn't work in IE, so work-around (Ben)
	    //return el['namespaceURI'] + el['localName'];

	    var match = el.nodeName.match(/([^:]+)$/);
	    return el['namespaceURI'] + match[1];
	}
	...
	while (frame['parent']) {
		...
		if (frame['parent'] && frame['parent']['collection']) {
		    ...
		    // we need a node
		    var about = XML_getAttributeNodeNS(frame, dom,
			RDFParser['ns']['RDF'],"about");
		    var rdfid = XML_getAttributeNodeNS(frame, dom,
			RDFParser['ns']['RDF'],"ID");
		    if (about && rdfid) {
			...
			var bnid = XML_getAttributeNodeNS(frame, dom,
		    ...
		    // Typed nodes
		    var rdftype = XML_getAttributeNodeNS(frame, dom,
		    ...
		    var parsetype = XML_getAttributeNodeNS(frame, dom,
			RDFParser['ns']['RDF'],"parseType")
		    var datatype = XML_getAttributeNodeNS(frame, dom,
		    ...
		    if (attrs['length'] != 0) {
			var resource = XML_getAttributeNodeNS(frame, dom,
	...
	// build up the namespaces for IE (Ben)
	frame_build_ns(parent, frame, element);
